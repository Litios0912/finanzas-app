...
import { gastosFijosDb, categoriasDb } from '../services/db'

const COLORS = { primary: '#4F46E5', danger: '#EF4444', bg: '#F3F4F6', cardBg: '#fff', text: '#111827', textSecondary: '#6B7280' }
...
export default function GastosFijosScreen() {
  ...
  const cargar = useCallback(async () => {
    ...
    const [gRes, cRes] = await Promise.all([
      gastosFijosDb.listar(familiaId),
      categoriasDb.listar(familiaId),
    ])
    setGastos(gRes.data || [])
    setCategorias((cRes.data || []).filter(c => c.tipo === 'fijo'))
    setLoading(false)
  }, [familiaId])
  ...
  const abrirModal = (gasto = null) => {
    if (gasto) {
      ...
      setDiaPago(String(gasto.dia_pago))
  ...
  const guardar = async () => {
    ...
    setGuardando(true)
    try {
      const datos = { familia_id: familiaId, nombre, monto: parseFloat(monto), dia_pago: parseInt(diaPago), categoria_id: categoriaId, activo }
      if (editando) {
        await gastosFijosDb.actualizar(editando.id, { nombre, monto: parseFloat(monto), dia_pago: parseInt(diaPago), categoria_id: categoriaId, activo })
      } else {
        await gastosFijosDb.crear(datos)
      }
  ...
  const eliminar = (id) => {
    Alert.alert('Eliminar', '¿Eliminar este gasto fijo?', [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: async () => {
        await gastosFijosDb.eliminar(id)
        await cargar()
      }},
    ])
  }
      ...
      >
        {gastos.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>Sin gastos fijos</Text>
            <Text style={styles.emptySubtext}>Agrega renta, suscripciones, seguros...</Text>
          </View>
        ) : (
          gastos.map(gasto => (
            <TouchableOpacity key={gasto.id} style={styles.gastoItem} onPress={() => abrirModal(gasto)}>
              <View style={styles.gastoInfo}>
                ...
                <Text style={styles.gastoMeta}>Día {gasto.dia_pago} • {gasto.activo ? 'Activo' : 'Inactivo'}</Text>
              </View>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{editando ? 'Editar Gasto Fijo' : 'Nuevo Gasto Fijo'}</Text>

            <TextInput style={styles.input} placeholder="Nombre (ej: Netflix)" value={nombre} onChangeText={setNombre} />
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoriaRow}>
              {categorias.map(cat => (
                ...
                >
                  <Text style={styles.categoriaText}>{cat.icono} {cat.nombre}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
            ...
            <View style={styles.switchRow}>
              ...
              <Switch value={activo} onValueChange={setActivo} trackColor={{ false: '#D1D5DB', true: '#A5B4FC' }} thumbColor={activo ? COLORS.primary : '#9CA3AF'} />
            </View>
...
const styles = StyleSheet.create({
  ...
  gastoMonto: { fontSize: 16, fontWeight: 'bold', color: COLORS.danger, marginRight: 8 },
  deleteBtn: { padding: 4 },
  ...
  categoriaChipActive: { backgroundColor: '#EEF2FF', borderColor: COLORS.primary },
